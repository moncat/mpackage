...
! function () {
    ...
    a.v = "1.2", b = document, c = "getElementById", d = "getElementsByTagName", e = 0, f = function (a) {
        ...
        return c.pages > c.groups && e.end < c.pages && c.last && 0 !== c.groups && d.push('<span>…</span><a href="javascript:;" class="laypage_last" title="尾页"  data-page="' + c.pages + '">' + c.last + "</a>"), e.flow = !c.prev && 0 === c.groups, (c.curr !== c.pages && c.next || e.flow) && d.push(function () {
            return e.flow && c.curr === c.pages ? '<span class="page_nomore" title="已没有更多">' + c.next + "</span>" : '<a href="javascript:;" class="laypage_next" data-page="' + (c.curr + 1) + '">' + c.next + '</a>'+getSelected(c.limit)
        ...
        $(select).on("change", function () {
           c.limit = $(this).find('option:selected').prop('value');
           (c.curr = 1, b.render())
        })        
...
function getSelected(limit){
	var html = '<select style="height: 28px;border: 1px solid #ccc;background-color: #fff;width: 94px;">';
	var data1 ='';
	var data2 ='';
	var data3 ='';
	var data4 ='';
	...
	var data6 ='';
	var data7 ='';
	if(limit ==null || limit ==undefined  || limit ==15){ var data1 = 'selected'; }
	if( limit ==30){ var data2 = 'selected'; }else
	if( limit ==50){ var data3 = 'selected'; }else
	if( limit ==100){ var data4 = 'selected'; }else
	if( limit ==200){ var data5 = 'selected'; }else
	if( limit ==500){ var data6 = 'selected'; }else
	if( limit ==1000){ var data7 = 'selected'; }else { var data1 = 'selected'; }
	html+='<option value="15" '+data1+' >15 条/页</option>';
	html+='<option value="30" '+data2+' >30 条/页</option>';
	html+='<option value="50" '+data3+' >50 条/页</option>';
	html+='<option value="100" '+data4+' >100 条/页</option>';
	html+='<option value="200" '+data5+' >200 条/页</option>';
	html+='<option value="500" '+data6+' >500 条/页</option>';
	html+='<option value="1000" '+data7+' >1000 条/页</option>';
	html += '</select>';
	return html;
}
