layui.use(['layer', 'form', 'element', 'jquery', 'dialog'], function() {
	...
	$('#comparisonDiv').on('mouseover', function() {
		
		
		var str = hls("get", "contrastProduct");
		if(str != false){
			$('#contrastProduct').text("产品对比("+JSON.parse(str).length+")");
		}
		
		var str2 = hls("get", "contrastBrand");
		if(str2 != false){
			$('#contrastBrand').text("品牌对比("+JSON.parse(str2).length+")");
		}
		
		var str3 = hls("get", "contrastEnterprise");
		if(str3 != false){
			$('#contrastEnterprise').text("企业对比("+JSON.parse(str3).length+")");
		}
		
		var str4 = hls("get", "contrastIngredient");
		if(str4 != false){
			$('#contrastIngredient').text("成分对比("+JSON.parse(str4).length+")");
		}
